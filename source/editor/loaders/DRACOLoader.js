...
class DRACOLoader extends Loader {
		...
		return this.decodeGeometry(buffer, taskConfig)
			...
			.catch((error) => {
				if(typeof onError === 'function') {
					...
					console.error('THREE.DRACOLoader:', error);
				}
			});
	...
	decodeGeometry(buffer, taskConfig) {
		...
		// Defensive Check: If an external loader called this with a single argument, populate defaults
		if(!taskConfig) {
			...
			// If it passed an object but lacks the properties (e.g. legacy GLTFLoader signatures)
			taskConfig.attributeIDs = taskConfig.attributeIDs || this.defaultAttributeIDs;
			taskConfig.attributeTypes = taskConfig.attributeTypes || this.defaultAttributeTypes;
...
function DRACOWorker() {
			...
			case 'decode':
				...
				decoderPending.then((module) => {
					...
					try {
						...
						// Pass the buffer array directly as the modern worker expects
	...
	function decodeGeometry(draco, decoder, array, taskConfig) {
		...
		decoderBuffer.Init(new Int8Array(array.buffer), array.byteLength);
		...
		if(geometryType === draco.TRIANGULAR_MESH) {
			...
			if(typeof decoder.DecodeArrayToMesh === 'function') {
			...
			if(typeof decoder.DecodeArrayToPointCloud === 'function') {
