...
class Script extends Group
{
	 ...
	 */
	async compileCode(code, onReady)
	{
		try
		{
			...
			if(this.mode === Script.APPEND)
			{
				...
				for(var i = 0; i < libs.length; i++)
				{
					...
					if(libCode === null)
					{
						libCode = await FileSystem.readFile(libs[i], true);
						if(libCode !== null)
						{
							code = libCode + "\n" + code;
						}
...
if(text === null)\
{\
text = await FileSystem.readFile(name, true);\
if(text !== null)\
{\
			...
			else if(this.mode === Script.INCLUDE)
			{
				...
				for(var i = 0; i < libs.length; i++)
				{
					...
					else
					{
						...
						var text = await FileSystem.readFile(libs[i], true);
						if(text !== null)
						{
