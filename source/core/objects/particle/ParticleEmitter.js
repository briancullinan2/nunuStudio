...
class ParticleEmitter extends Points
{
	/**
	 * @constructor
	 * @param {ParticleGroup} group Fully initialized group instance passed by the factory.
	 * @param {Object} emitter Configurations or instance for the emitter control.
	 */
	...
	{
		const { ParticleEmitterControl } = await import("./core/ParticleEmitterControl.js");
		 ...
		 * Emitter instance.
		 *
		 * Emitter has attributes that can be used to control the particle system
		 *
		 * @property emitter
		...
		/**
		 * A dynamic particle emitter ignores the position in its transform and applies it directly to the emitter origin.
		 *
		 * @property dynamicEmitter
		 * @type {boolean}
		 */
	...
	/**
	 * Asynchronous static factory to handle dynamic module imports and initialization.
	 * * @static
	 * @method create
	 * @param {Object} [group] Configuration for the particle group.
	 * @param {Object} [emitter] Configuration for the particle emitter.
	 * @return {Promise<ParticleEmitter>}
	 */
	static async create(group, emitter)
	{
		const { ParticleGroup } = await import("./core/ParticleGroup.js");

		const grp = new ParticleGroup(group !== undefined ? group : ParticleEmitter.defaultGroup);

		// Return a fresh instance with async dependencies fulfilled cleanly
		return new ParticleEmitter(grp, emitter);
	}
